...
- **Database design** — normalized 8-entity relational schema (PostgreSQL): Users, Roles, Vehicles, Drivers, Trips, Maintenance Logs, Fuel Logs, Expenses. Native enums for all statuses, unique constraints on registration/license numbers, composite indexes on every hot query path (`Trip(vehicleId,status)`, `Vehicle(status)`, `FuelLog(vehicleId,date)`, …).
  - *Design decision — Roles:* the Roles entity is implemented as a PostgreSQL **enum type** (`Role`) on the Users table rather than a lookup table. The system has exactly four fixed roles with behavior compiled into the RBAC permission matrix (`src/lib/auth.ts`); an enum gives the same referential integrity as a foreign key with one less join on every authenticated request, and invalid roles are rejected by the database itself. If dynamic, admin-defined roles were ever needed, promoting the enum to a table is a single additive migration.
...
- **Security** — bcrypt password hashing, httpOnly/sameSite JWT cookies, RBAC enforced server-side on every write, login rate limiting (10 attempts / 15 min), security headers (X-Frame-Options, nosniff, referrer & permissions policies), least-privilege DB role, secrets only in gitignored `.env`.
  - *Design decision — public tracking page:* the entire application requires authentication, with one deliberate, tightly-scoped exception: the customer shipment-tracking page (`/track/<ref>`), mirroring the industry-standard courier tracking link. It is read-only, addressable only by trip reference number, and its database query selects exclusively non-sensitive fields — no phone numbers, license data, revenue or cost figures ever leave the server for that route. Every operational screen and every API endpoint remains behind authentication.
